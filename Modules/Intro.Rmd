...
closed the same way. ' can be used if the string contains " and vice versa " 
can be used in the string contains '. Otherwise it is user preference. 
...
```{r}
print("Don't use single if there's an apostorphe")
...
## Data Objects  
Data bjects are assigned with the assigment operator '<-' or '->'. The arrow points
to the object name. 
...
Objects can  be assigned in succession. Assigment can also be done inside
expressions. 
```{r}
x <- y <- z <- 5
...
R for use in the current R session library(package_name). For install.packages()
